...
:root{--navy-900:#1E3A5F;--navy-800:#2C5282;--navy-700:#3182CE;--canvas:#F7F9FB;--border:#E2E8F0;--border-strong:#CBD5E1;--text:#0F172A;--muted:#64748B;--shadow:0 2px 10px rgba(15,23,42,.05);--shadow-md:0 8px 24px rgba(15,23,42,.08);--shadow-lg:0 18px 45px rgba(15,23,42,.12);}
...
.login-brand{background:linear-gradient(150deg,var(--navy-900),var(--navy-800));padding:48px;color:#FFF;display:flex;flex-direction:column;justify-content:center;min-height:510px;}
...
def get_user_supabase_client(user_email):
    if not JOSE_AVAILABLE: return supabase
    try:
...
def show_home(user, rls_client):
    ...
    with a: st.markdown(f"<div class='kpi-card'><div class='kpi-label'>Circulars & G.O.s</div><div class='kpi-value'>{len(circulars)}</div><div class='kpi-foot'>Total visible to you</div></div>", unsafe_allow_html=True)
    ...
    if not recent: st.info("No circulars published yet.")
        ...
        for c_ in recent:
            ...
            st.markdown(
                f'<div class="doc-card"><div class="doc-row"><span class="doc-ref">{esc(c_.get("ref_id"))}</span>{tier_badge(c_.get("tier_required", "Basic"))}</div>'
            ...
            elif not allowed: st.caption(f"🔒 Requires {safe_str(c_.get('tier_required'))} access or higher")
...
def show_circulars(user, rls_client):
    ...
    for r in rows:
        ...
        st.markdown(
            f'<div class="doc-card"><div class="doc-row"><span class="doc-ref">{esc(r.get("ref_id"))}</span>{tier_badge(r.get("tier_required", "Basic"))}</div>'
        ...
        with col1:
            ...
            elif not allowed: st.warning(f"🔒 Requires {safe_str(r.get('tier_required'))} access or higher")
...
def show_tapal(user, rls_client):
    ...
    with t1:
        ...
        with st.form("tapal_form", clear_on_submit=True):
            ...
            with b:
                ...
                status = st.selectbox("Status", ["Received", "Registered", "Assigned", "In Progress", "Awaiting Reply", "Closed"])
            with c:
    ...
    with t3:
        ...
        if df.empty: st.info(f"No entries for {start.strftime('%B %Y')}.")
            ...
            cols = [c_ for c_ in ["r_no", "tapal_date", "direction", "from_to", "subject", "status", "section", "remarks"] if c_ in df.columns]
...
def show_directory(user, rls_client):
    ...
    rows = fetch_directory(rls_client)
    ...
    if section == "👥 Users":
        ...
        with st.form("add_go_form"):
            ...
            if st.form_submit_button("Publish Document", type="primary", use_container_width=True):
                ...
                if rls_client.table("circulars").select("id").eq("ref_id", ref_id).execute().data: errs.append(f"'{ref_id}' already exists.")
